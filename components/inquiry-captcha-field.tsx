...
export function InquiryCaptchaField({
  ...
  useEffect(()=>{setScope(createInquiryCaptchaScope())},[])
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (!requests.isCurrent(request)) return
      setError('Unable to load the code. Select Refresh to retry.')
    } finally {
  ...
  return (
    <div className={className}>
      <label htmlFor={`${answerName}-${scope}`} className="mb-2 block text-sm font-medium">
        Verification code
      </label>
      <div className="flex flex-wrap items-center gap-3">
        ...
        <input
          ...
          spellCheck={false}
          inputMode="text"
          aria-describedby={error ? `${answerName}-${scope}-error` : undefined}
      ...
      {error ? (
        <p id={`${answerName}-${scope}-error`} role="alert" className="mt-2 text-sm text-red-700">
          {error}
        </p>
      ) : null}
    </div>
